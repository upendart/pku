...
class sspmod_saml_Message {
	...
	private static function processAssertion(
		...
		// CLB HACKED - adjusted + 60 to + 65 because Salesforce server is 2 minutes ahead??? Not sure why but that's the case.
		if ($notBefore !== NULL && $notBefore > time() + 65) {
			$timediff = $notBefore .' > '. (time() + 65);
			throw new SimpleSAML_Error_Exception('Received an assertion that is valid in the future. Check clock synchronization on IdP and SP. ['.$timediff.']');
		}
